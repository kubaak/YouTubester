    ...
    : IVideoTemplatingService
{
        ...
        CopyVideoTemplateRequest request, CancellationToken cancellationToken)
    {
        ...
        var source = await youTubeIntegration.GetVideoDetailsAsync(sourceId, cancellationToken);
        if (source == null)
        {
            throw new ArgumentException($"Source {sourceId} not found.");
        }
        var target = await youTubeIntegration.GetVideoDetailsAsync(targetId, cancellationToken);
        if (target == null)
        {
            throw new ArgumentException($"Target {targetId} not found.");
        }
    ...
    private static string ParseYouTubeVideoId(string urlOrId)
    {
        ...
        var s = urlOrId.Trim();
        if (s.Length == 11 && !s.Contains('/')) return s;  // likely already an ID
