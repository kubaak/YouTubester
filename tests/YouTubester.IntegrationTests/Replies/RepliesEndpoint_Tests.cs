...
namespace YouTubester.IntegrationTests.Replies;

[Collection(nameof(TestCollection))]
public class RepliesEndpoint_Tests(TestFixture fixture)
{
    ...
    public async Task BatchApprove_ValidDecisions_ReturnsSuccessResult()
    {
        ...
        var decisions = new[]
        {
            new DraftDecisionDto("comment1", "Approved text 1"),
            new DraftDecisionDto("comment2", "Approved text 2")
        };
    ...
    public async Task BatchIgnore_ValidCommentIds_ReturnsSuccessResult()
    {
        ...
        result.IgnoredIds.Should().BeEquivalentTo(new[] { "comment1", "comment2" });
        result.SkippedPostedIds.Should().BeEquivalentTo(new[] { "comment3" });
        result.NotFoundIds.Should().BeEquivalentTo(new[] { "non-existent" });
