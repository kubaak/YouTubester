...
public class VideosEndpoint_FullTests(TestFixture fixture)
{
    ...
    public async Task CopyTemplate_ValidRequest_ReturnsJobId()
    {
        ...
        var enqueuedJobs = fixture.ApiFactory.CapturingJobClient.GetEnqueued<YouTubester.Application.Jobs.CopyVideoTemplateJob>();
    ...
    [Fact]
    public async Task SyncVideos_ExistingChannel_ReturnsSuccessResult()
    {
        // Arrange
        await fixture.ResetDbAsync();

        var testChannelName = "TestChannel";
        ...
        // Mock the video service to return a sync result
        fixture.ApiFactory.MockYouTubeIntegration
            ...
            .Returns(CreateAsyncEnumerable(new List<YouTubester.Integration.Dtos.VideoDto>()));

        // Act
        var response = await fixture.HttpClient.PostAsync($"/api/videos/sync/{testChannelName}", null);

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK);

        var responseContent = await response.Content.ReadAsStringAsync();
        var result = JsonSerializer.Deserialize<SyncVideosResult>(responseContent, _serializerOptions);
    ...
    [Fact]
    public async Task SyncVideos_NonExistentChannel_ReturnsNotFound()
    {
        // Arrange
        await fixture.ResetDbAsync();

        // Act
        var response = await fixture.HttpClient.PostAsync("/api/videos/sync/NonExistentChannel", null);

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.NotFound);

        var responseContent = await response.Content.ReadAsStringAsync();
        responseContent.Should().Contain("Channel 'NonExistentChannel' not found");
    }
    ...
    public async Task GetVideos_WithVideosInDb_ReturnsFilteredAndPagedResults()
    {
        ...
        var visibilityResult = JsonSerializer.Deserialize<PagedResult<VideoListItemDto>>(visibilityContent, _serializerOptions);
        ...
        var paginationResult = JsonSerializer.Deserialize<PagedResult<VideoListItemDto>>(paginationContent, _serializerOptions);
