...
public class Reply
{
    ...
    {
        EnsureNotPosted();
        if (Status == ReplyStatus.Approved) throw new InvalidOperationException("Already approved.");
    ...
    {
        EnsureNotPosted();
        if (Status != ReplyStatus.Approved) throw new InvalidOperationException("Approve before posting.");
        if (string.IsNullOrWhiteSpace(FinalText)) throw new InvalidOperationException("FinalText required.");
    ...
    public void Ignore()
    {
        if (Status == ReplyStatus.Posted) throw new InvalidOperationException("Already posted.");
    ...
    {
        if (PostedAt is not null) throw new InvalidOperationException("Reply is already posted.");
    }
